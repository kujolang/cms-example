...
The administration API is authenticated and capability-gated. Users, profiles, roles, approval state, social links, and PBKDF2 password credentials are durable records in the Kujo CMS database. Administrators can create and edit users, approve or reject registrations, suspend accounts, reset passwords, and choose whether public signup is open, approval-based, or closed. Every signed-in user has a self-service `/account` page. Local development demonstrates HTTP-only, SameSite cookies containing bounded, revocable CMS session tokens; hosted deployments can exchange a trusted platform identity for the same core user and capability model. The browser never receives password hashes or the CMS write token. Image uploads are resized and converted to WebP in the browser, then signature-verified and stored through the CMS media library; production deployments can register objects verified by S3, GCS, Azure, R2, or custom storage adapters.
